...
**v3.1 재정의 메모**: v3 원안은 "customers 테이블에서..."로 시작해 GuardViolation을 강제 유발하는 구조였음. D-6 실측 과정에서 3가지 약점이 확인되어 이 축을 `(c) 자가교정`에서 분리, 신설된 `(f) 거버넌스 내재화`로 재분류. 상세는 문서 하단 "v3 → v3.1 변경 이력" 참조. 진짜 `is_error=True` 피드백 루프 시연은 **DB 타입 에러 복구 사례**로 이전 (`docs/interview-prep/design-decisions.md §7.1` 예정).
...
- Q: **"그럼 자가교정 기능은 따로 없나요?"**  
  → A: "있습니다. 단 두 축으로 분리했습니다. **거버넌스 경로 선택(Scene 2)**은 safe-by-default로 에이전트에 내재화되어 있고, **진짜 `is_error=True` 피드백 루프**는 D-6 실행 중 실제 발생한 **DB 타입 에러 복구 사례**로 보여드릴 수 있습니다 — `ROUND(double precision, integer)` PostgreSQL 시그니처 오류를 Agent가 감지해 CAST를 추가해 재시도한 실측 케이스입니다. 상세는 design-decisions §7.1을 참고해주세요."
...
| **자가교정 실사례** (DB 에러) | 면접 Q&A 별도 시연 (design-decisions §7.1 예정) |
...
**4. Resilience 축 이전**: `is_error=True` 피드백 루프의 실측 시연은 DB 타입 에러 복구 케이스로 이전. 상세 케이스 기록은 `docs/interview-prep/design-decisions.md §7.1` (D-6 내 추가 예정).
...
| 지적 (만장일치 4/4) | 반영 위치 |
|---|---|
| Self-correction 핵심 질문 필요 | **DB 에러 복구 실측 케이스 (design-decisions §7.1)** — v3.1에서 C4·Scene 2 시나리오 유발 구조는 폐기 |
| 단순 조회형 배제 | 전체 재설계 |
