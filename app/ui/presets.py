"""
골든셋 v3.1 프리셋 질문 — Streamlit 메인 영역 카드 시연용.
...
dataclass 필드:
- key/label/question/scene/note: 기존 (모든 호출부 backward compatible)
- title: 카드 헤더에 쓰는 코드 제외 짧은 제목 (없으면 label에서 추출)
- badges: 카드 우측 표시용 짧은 태그 튜플 (예: "추천", "조회통제")
"""
...
class Preset:
    key: str                    # C1, C2, ... / S3-T1
    label: str                  # 기존 짧은 라벨 (사이드바·continuation chip 호환용)
    question: str               # Agent로 전달될 질문
    scene: str | None           # "Scene 1", "Scene 2", "Scene 3" or None
    note: str | None            # 시연 시 멘트 (툴팁)
    title: str = ""             # 카드 헤더용 짧은 제목
    badges: tuple[str, ...] = field(default_factory=tuple)  # 카드 표시 배지
...
SCENE_3_MULTITURN: list[Preset] = [
    ...
    Preset("S3-T2", "Scene 3 · T2 GA 드릴다운",
           ...
           "Scene 3", "T1 답변 맥락(GA)을 이어받아 상품 단위로 드릴다운",
           title="T2 GA 드릴다운",
           badges=("멀티턴",)),
